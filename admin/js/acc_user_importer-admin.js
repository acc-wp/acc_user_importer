...
(function ($) {
  ...
  function wpEstablishLocalAPI(successFn, failureFn) {
    ...
    jQuery.post(ajax_object.url, apiData, function (response) {
      ...
      if (responseObject.message == "established") {
        ...
        logLocalOutput(
          "Error: A local API connection could not be established.",
        );
  ...
  function wpRequestChangedMembers(successFn, failureFn) {
    ...
    jQuery.post(ajax_object.url, apiData, function (response) {
      ...
      if (responseObject.message == "success") {
        logLocalOutput(
          "Number of members with changes: " + responseObject.count,
        );
  ...
  function getNextDataset(changeList, dataOffset, apiAttemptsRemaining) {
    ...
    if (dataOffset > changeList.length) {
      logLocalOutput(
        "Error: attempt to getNextDataset with the changeList fully processed",
      );
      return onPostRequestFailure();
    }
    ...
    wpRequestACCData(
      ...
      function (changeList, dataOffset, memberArray) {
        //Parse Data
        wpProccessMembershipData(
          changeList,
          dataOffset,
          memberArray,
          function (changeList, dataOffset) {
            ...
            //Enable Buttons At End
              ...
              logLocalOutput(
                "--Parsed data for " + usersInData + " people total.",
              );
              logLocalOutput(
                "--Created accounts for " + newUsers + " people total.",
              );
              logLocalOutput(
                "--Updated data for " + updatedUsers + " people total.",
              );
  ...
  function wpRequestACCData(changeList, dataOffset, successFn, failureFn) {
    ...
    jQuery.post(ajax_object.url, apiData, function (response) {
      ...
      if (responseObject.message == "success") {
        ...
        if (successFn)
          successFn.call(this, changeList, nextOffset, responseObject.results);
      } else {
        logLocalOutput(
          "Error: " +
            (responseObject.errorMessage
   ...
   */
  function wpProccessMembershipData(
    changeList,
    dataOffset,
    memberArray,
    successFn,
  ...
  function wpProccessExpiry() {
    ...
    jQuery.post(ajax_object.url, apiData, function (response) {
      ...
      if (responseObject.message == "success") {
        ...
        var duration =
          (accSyncEndTime.getTime() - accSyncStartTime.getTime()) / 1000;
        logLocalOutput("Start time: " + accSyncStartTime);
